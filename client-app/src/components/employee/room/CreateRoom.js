...
const CreateRoom = ({ isShowing, toggle, onSuccess }) => {
    const time = new Date();
    time.setHours(new Date().getHours() + 7);
    ...
    const [timeStart, setTimeStart] = useState(strTime.slice(0, strTime.length - 8));
    ...
    const handleCreateRoom = () => {
        createRoom(
            roomName, subjectId, teacherId, year, semester,
            new Date(timeStart).getTime() / 1000, Number(duration)
        ).then(response => {
    ...
    const modal = (
        <Modal isShowing={isShowing} >
            <div className="flex">
                <p className="font-semibold text-xl text-gray-600">Thêm phòng thi</p>
